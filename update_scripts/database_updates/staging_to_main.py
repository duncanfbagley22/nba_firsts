...
def check_date():
    ...
    date_span = soup.find('span', class_='button2 index')
    ...
    # Extract the date text (e.g., 'Jan 31, 2025')
    date_text = date_span.text.strip()

    # Convert the extracted date to a datetime object
    page_date = datetime.strptime(date_text, "%b %d, %Y").date()

    # Step 4: Get yesterday’s date
    yesterday = datetime.today().date() - timedelta(days=1)

    if page_date != yesterday:
        print("Date does not match yesterday. Exiting script.")
        exit()
